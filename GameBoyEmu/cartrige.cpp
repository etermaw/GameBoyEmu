...
void Cartrige::dispatch()
{
	...
	u32 rom_banks = 2 << header->rom_size;
	...
	{
		auto tmp = std::make_unique<MBC1>(MBC1(rom.get(), ram.get(), rom_banks));
		dma_interface = tmp.get();
		memory_interface = std::move(tmp);
	}
	...
	{
		auto tmp = std::make_unique<MBC2>(MBC2(rom.get(), ram.get(), rom_banks));
		dma_interface = tmp.get();
		memory_interface = std::move(tmp);
	}
	...
	{
		auto tmp = std::make_unique<MBC3>(MBC3(rom.get(), ram.get(), (type <= 0x10 ? rtc_regs : nullptr), rom_banks));
		dma_interface = tmp.get();
		memory_interface = std::move(tmp);
	}
	...
	{
		auto tmp = std::make_unique<MBC5>(MBC5(rom.get(), ram.get(), rom_banks));
		dma_interface = tmp.get();
		memory_interface = std::move(tmp);
	}
