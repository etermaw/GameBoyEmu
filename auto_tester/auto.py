...
async def worker_func(file_names, wait_time, result_dict):
    for file_path in file_names:
        file_name = os.path.basename(file_path)
        result_dict[file_name] = None  # if any crash happen, None will mark it

        process = await asyncio.create_subprocess_exec('./auto_test', file_path, stdin=asyncio.subprocess.PIPE, stdout=asyncio.subprocess.PIPE)
...
if len(sys.argv) != 5:
    print('Args: <result file> <top test directory> <number of parallel tests> <wait time for tests>')
    ...
    results = dict()
    tests_to_run = asyncio.gather(*[worker_func(w, sleep_time, results) for w in work_chunks], return_exceptions=True)
